...
```{r}
if ( is_installed("networkD3") ){
  networkD3::simpleNetwork(edges_df)
}
```
...
nodes_df <- 
  unique(
    data.frame(
      node_id  = c(edges_df$node_1, edges_df$node_2),
      group_id = c(edges_df$subgraph_id, edges_df$subgraph_id) * 2
    )
  )

nodes_df <- 
  nodes_df[order(nodes_df$node_id),]
